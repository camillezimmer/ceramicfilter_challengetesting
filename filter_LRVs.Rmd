...
raw_LRV = read_excel("LRV_ceramicfilters.xlsx", col_names = T, skip = 1)
```
...
summ_LRVstats = raw_LRV %>%
  ...
  mutate(label = c("= 7.7 (95% CI 7.6 - 7.8; n = 12)", "= 7.8 (95% CI 7.5 - 8.1; n = 13)"))
summ_LRVstats
...
#Name panels
strainID_labs = c("Mutaflor in stationary phase", "K-12 in stationary phase")
names(strainID_labs) = c("M", "K")

#Generate colours
colours = c("#C63527","#005493")

# Set display order of the beginning/end of life box plots
raw_LRV$filter_stage = factor(raw_LRV$filter_stage, levels = c("N","E"))
...
plot = ggplot(raw_LRV) +
  aes(x = block_ID, y = LRV, fill = filter_stage) +
  geom_boxplot(alpha = 0.5) +
  geom_jitter(colour= "BLACK") +
  scale_fill_manual(values = colours, breaks = c("N","E"), labels = c("Beginning of life","End of life"), name = "Filter stage")+
  # scale_fill_brewer(palette="Greys", breaks = c("N","E"), labels = c("Beginning of life","End of life"), name = "Filter stage")+    # Uncommet this line for B+W colour scheme
  coord_flip() +
  theme_bw()+
  facet_wrap(vars(organism), scales = "free_y", ncol = 1, labeller = labeller(organism = strainID_labs)) +
  scale_y_continuous(name="LRV", breaks=seq(5.5,8.5,0.5)) +
  ...
  # geom_text(data = summ_LRVstats, label = summ_LRVstats$label)
...
```{r nonparametric regular stats for pub}
...
# YAAAAASSSS (K12 half life included)
#  p = 0.1556 (K12 half life excluded)
kruskal.test(LRV ~ run_ID, data = raw_LRV)
# Post-hoc checking --> no significant differences, yay!!!!
...
# Result: the control LRV 95% CI included 0
...
##### Plot all factors ----
# ggplot(raw_LRV) +
#   aes(x = run_ID, y = LRV, fill = filter_stage) +
#   geom_boxplot() +
#   geom_jitter() +
#   scale_fill_manual(
#     values = c(E = "#F8766D",
...
```{r try  stats using censored value functions}
str(raw_LRV)
